    ...
    def call(self, inputs):
        ...
        outputs = utils.batch_slice(
            ...
            lambda w, x, y, z: detection_targets_graph(
                w, x, y, z, self.config),
            (proposals.dtype, gt_class_ids.dtype, proposals.dtype, proposals.dtype),
            self.config.IMAGES_PER_GPU)
        return list(outputs)
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        return tf.reshape(
            detections_batch,
            tf.stack([tf.shape(image_meta)[0], self.config.DETECTION_MAX_INSTANCES, 6]))

    def compute_output_shape(self, input_shape):
        return (input_shape[0][0], self.config.DETECTION_MAX_INSTANCES, 6)
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir, sess=None, create_log_dir=True):
        """
        ...
        self.set_log_dir(create_log_dir=create_log_dir)
    ...
    def build(self, mode, config):
        ...
        rpn = build_rpn_model(config.RPN_ANCHOR_STRIDE,
                              len(config.RPN_ANCHOR_RATIOS), config.TOP_DOWN_PYRAMID_SIZE + int(constants.DETECTION_USE_ANGLE))
    ...
    def find_last(self):
        ...
        """
        import glob
        checkpoints = sorted(glob.glob(os.path.join(self.model_dir, '*', '*.h5')), key=os.path.getmtime)
        '''
        ...
        if not checkpoints:
            import errno
            raise FileNotFoundError(
                errno.ENOENT, "Could not find weight files in {}".format(self.model_dir))
        checkpoint = checkpoints[-1]
        return checkpoint

    def load_weights(self, filepath, by_name=False, exclude=None, create_log_dir=False):
        ...
        self.set_log_dir(filepath, create_log_dir=create_log_dir)
    ...
    def set_log_dir(self, model_path=None, create_log_dir=True):
        ...
        if create_log_dir and not os.path.exists(self.log_dir):
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        def linear_decay(epoch):
            if epoch < self.config.START_LINEAR_INCREASE:
                lr = self.config.MIN_LR
            elif self.config.START_LINEAR_INCREASE <= epoch < self.config.END_LINEAR_INCREASE:
                lr = self.config.MAX_LR * (epoch - self.config.START_LINEAR_INCREASE) + self.config.MIN_LR * (self.config.END_LINEAR_INCREASE - epoch)
                lr /= (self.config.END_LINEAR_INCREASE - self.config.START_LINEAR_INCREASE)
            else:
                lr = self.config.MIN_LR * (epoch - self.config.END_LINEAR_INCREASE) + self.config.MAX_LR * (self.config.END_LINEAR_DECREASE - epoch)
                lr /= (self.config.END_LINEAR_DECREASE - self.config.END_LINEAR_INCREASE)
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path,
                                            ...
                                            period=1),
            #keras.callbacks.LearningRateScheduler(linear_decay)
        ]
        ...
        self.keras_model.fit_generator(
            ...
            callbacks=callbacks,
            validation_data=None,#val_generator,
    ...
    def save_model(self):
        ...
        if len(checkpoints) > 0:
            regex = r".*(\d{4})\.h5"
            m = re.match(regex, checkpoints[-1])
            if m:
                epoch = int(m.group(1))
    ...
    def detect(self, images, angles=None, verbose=0):
        ...
        detections, _, _, mrcnn_mask, _, _, _ = self.keras_model.predict_function(
    ...
    def detect_molded(self, molded_images, image_metas, verbose=0):
        ...
        # TODO: can this be optimized to avoid duplicating the anchors?
        #anchors = np.broadcast_to(anchors, (self.config.BATCH_SIZE,) + anchors.shape)
        anchors = np.broadcast_to(anchors, (len(molded_images),) + anchors.shape)
